...
void PreProcess(const pcl::PointCloud<pcl::PointXYZI>& cloudIn, pcl::PointCloud<PointCloudXYZRDP>& cloudOut){
	...
	total_frame = int(cloudIn.points.size() / 64) - 1;

	cout<<"当前帧点云总数为：" << cloudIn.points.size()<<endl;
	...
	for (int j = 0; j < total_frame; ++j) {
		int num_odd = 32;                //奇数从32位开始排
		int num_even = 0;                //偶数从头

		for (int i = 0; i < 64; ++i) {
			if (float(i % 2) == 0.0) {
				cloudOut.points[j * 64 + num_even].x = cloudIn.points[j * 64 + i].x ;
				cloudOut.points[j * 64 + num_even].y = cloudIn.points[j * 64 + i].y ;
				cloudOut.points[j * 64 + num_even].z = cloudIn.points[j * 64 + i].z ;
				cloudOut.points[j * 64 + num_even].ring =num_even;
				cloudOut.points[j * 64 + num_even].pcaketnum =j;
				cloudOut.points[j * 64 + num_even].range =cloudIn.points[j * 64 + i].intensity;
	
				pcl::PointXYZRGB p;
				p.x = cloudIn.points[j * 64 + i].x;
				p.y = cloudIn.points[j * 64 + i].y;
				p.z = cloudIn.points[j * 64 + i].z;
				p.r = 100;
				p.g = 100;
				p.b = 100;
				...
				cloudOut.points[j * 64 + num_odd].x  = cloudIn.points[j * 64 + i].x;
				cloudOut.points[j * 64 + num_odd].y = cloudIn.points[j * 64 + i].y ;
				cloudOut.points[j * 64 + num_odd].z = cloudIn.points[j * 64 + i].z ;
				cloudOut.points[j * 64 + num_odd].ring =num_odd;
				cloudOut.points[j * 64 + num_odd].pcaketnum =j;
				cloudOut.points[j * 64 + num_odd].range =cloudIn.points[j * 64 + i].intensity;
	...
	for(int j = 0; j < total_frame; ++j) {
		...
		for (int i = 1; i < 63; ++i) {
			if(!flag && cloudOut.points[j * 64 + i].range<=0.1){//小于0.1表示点缺失
				flag = true;
				p.first = i;
			}
			if(flag && cloudOut.points[j * 64 + i + 1].range > 0.1){
				p.second = i;
				flag = false;
				vt.push_back(p);
			}
		}
		//vt容器的个数表示扫描中点缺失的个数，利用线性差值补全
		for(int i=0; i<vt.size(); ++i){
			//range小于0.1的点的range与第一个range大于0.1的点的range的差
			float dist = (cloudOut.points[j * 64 + vt[i].second + 1].range - cloudOut.points[j * 64 + vt[i].first - 1].range);
			//索引差
			float disring = vt[i].second - vt[i].first +1;
			float pre = cloudOut.points[j * 64 + vt[i].first - 1].range;

			float dx = (cloudOut.points[j * 64 + vt[i].second + 1].x - cloudOut.points[j * 64 + vt[i].first - 1].x)/disring;
			float dy = (cloudOut.points[j * 64 + vt[i].second + 1].y - cloudOut.points[j * 64 + vt[i].first - 1].y)/disring;
			float dz = (cloudOut.points[j * 64 + vt[i].second + 1].z - cloudOut.points[j * 64 + vt[i].first - 1].z)/disring;

			float x = cloudOut.points[j * 64 + vt[i].first - 1].x;
			float y = cloudOut.points[j * 64 + vt[i].first - 1].y;
			float z = cloudOut.points[j * 64 + vt[i].first - 1].z;
			if(fabs(dist) < 0.3){
				for(int k= vt[i].first; k<=vt[i].second ; ++k){
					...
					y += dy;
					z += dz;
					cloudOut.points[j * 64 + k ].range  = pre;
					cloudOut.points[j * 64 + k ].x = x;
					cloudOut.points[j * 64 + k ].y = y;
					cloudOut.points[j * 64 + k ].z = z;
					cloudOut.points[j * 64 + k ].ring =k;
					cloudOut.points[j * 64 + k ].pcaketnum =j;


					pcl::PointXYZRGB p;//将被补全的点设置为蓝色点
					p.x = cloudOut.points[j * 64 + k ].x;
					p.y = cloudOut.points[j * 64 + k ].y;
					p.z = cloudOut.points[j * 64 + k ].z;
					p.r = 0;
					p.g = 0;
					p.b = 255;
					colorcloud->points.push_back(p);		
...
void PreProcess2(const pcl::PointCloud<pcl::PointXYZI>& cloudIn, pcl::PointCloud<PointCloudXYZRDP>& cloudOut){

	total_frame = int(cloudIn.points.size() / 64);
	...
	for (int i = 0; i < 64; ++i) {
		for (int j = 0; j < total_frame; ++j) {
				cloudOut.points[j + i * total_frame].x = cloudIn.points[j + i * total_frame].x ;
				cloudOut.points[j + i * total_frame].y = cloudIn.points[j + i * total_frame].y ;
				cloudOut.points[j + i * total_frame].z = cloudIn.points[j + i * total_frame].z ;
				cloudOut.points[j + i * total_frame].ring =i;
				cloudOut.points[j + i * total_frame].pcaketnum =j;
				cloudOut.points[j + i * total_frame].range =cloudIn.points[j + i * total_frame].intensity;
	
				pcl::PointXYZRGB p;
				p.x = cloudIn.points[j + i * total_frame].x;
				p.y = cloudIn.points[j + i * total_frame].y;
				p.z = cloudIn.points[j + i * total_frame].z;
				p.r = 100;
				p.g = 100;
				p.b = 100;
				colorcloud->points.push_back(p);		
...
void toRangeImage(const pcl::PointCloud<PointCloudXYZRDP>& cloudIn, pcl::PointCloud<PointCloudXYZRDP>& cloudObstacle, pcl::PointCloud<PointCloudXYZRDP>& cloudGround, std::unordered_map<int, Range> &unordered_map_out){
	...
	float xmin = -30, xmax = 30, ymin = -50, ymax = 50, zmin = -2.0, zmax = 3.0;
        CloudFilter(cloudIn, *cloud_filtered, xmin, xmax, ymin, ymax, zmin, zmax);//剔除雷达周围过近及过远的

   	int64_t tm0 = gtm();
	GroundRemove ground_remove(3, 50, 0.5, 0.15);//(迭代次数，取z最小的点的数量，地面阈值，高度阈值)
	ground_remove.RemoveGround(*cloud_filtered, cloudGround, cloudObstacle);
   	int64_t tm1 = gtm();
   	printf("[INFO]ground remove cast time:%ld us\n",  tm1-tm0);
	
	//view
	cv::Mat image = cv::Mat::zeros(64, total_frame, CV_8UC3);
  	int scale = 200, min_dis = 1, max_dis = 30;


	for(int i = 0;  i< cloudObstacle.points.size(); ++i){
		int col = cloudObstacle.points[i].pcaketnum;
		int row = 63 - cloudObstacle.points[i].ring;
	...
	*/
	...
	for(int i = 0; i < cloudGround.points.size(); ++i){
		...
		int row = 63 - cloudGround.points[i].ring;
		//int row = cloudGround.points[i].ring;
...
std::vector<int> RangeSeg(const pcl::PointCloud<PointCloudXYZRDP>& cloudIn, pcl::PointCloud<PointCloudXYZRDP>& cloudbreak,std::unordered_map<int, Range> &unordered_map_out){
	...
	int direction[8][2] = {{0,1},{0,2},{0,-1},{0,-2},{-1,0},{1,0},{-2,0},{2,0}};

	int size = cloudIn.points.size();
	std::vector<int> cluster_indices(size, -1);

	float horizon_angle_resolution = 0.2 * PI / 180;
	float vertical_angle_resolution = 0.425 * PI / 180;
	...
	for(int i =0 ; i< (total_frame * 64); ++i){
		...
		for(int k = 0; k<4; ++k){//行不变 列+1 +2 -1 -2
			...
			float dnei = unordered_map_out[neighbor].range;
                        ...
                        float dmax = (dorigin) * sin(360/(float)total_frame*PI/180)/sin(20*PI/180 -360/(float)total_frame*PI/180) + 3*0.05;
		...
		for(int k = 4; k<8; ++k){//列不变，行+1 -1 
			int neighbor = (row + direction[k][0]) * total_frame + (col + direction[k][1]);
			if(!unordered_map_out.count(neighbor))
				continue;
			float dnei = unordered_map_out[neighbor].range;

                        float dmax = (dorigin) * sin(0.425*PI/180)/sin(40*PI/180 -0.425*PI/180) + 3;
			if(fabs(dnei-dorigin)<dmax  ) {//满足这个条件，这两个点按理说为同一类的点
				int oc = cluster_indices[unordered_map_out[i].index]; // original point's cluster
				int nc = cluster_indices[unordered_map_out[neighbor].index]; // neighbor point's cluster
				if (oc != -1 && nc != -1) {
					if (oc != nc)//类别冲突处理
		...
		if (cluster_indices[unordered_map_out[i].index] == -1) {
			...
			for(int k = 0; k<4; ++k){
				...
				float dnei = unordered_map_out[neighbor].range;
                        	float dmax = (unordered_map_out[i].range) * sin((360/(float)total_frame)*PI/180)/sin(20*PI/180 -(360/(float)total_frame)*PI/180) + 3*0.05;
...
bool most_frequent_value(std::vector<int> values, std::vector<int> &cluster_index) {
	...
	for (int i = 0; i < tr.size(); ++i) {
		if (tr[i].second > 10) {//聚类的点的数量大于10，则说明归为一类
	...
	for(int i=0; i<vt.size(); ++i){
		if (vt[i]->points.size() > 5) {
			...
			*/
			...
			viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, "cube" + toString(i));
			*/
		}
		
		viewer->addPointCloud(vt[i], "cloud2" + toString(i));
		//设置大小
		viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "cloud2" + toString(i));
	}
...
int main(int argc, char** argv) {
	...
	if(view){
		...
		for (int k = 0; k < cluster_id.size(); ++k) {//聚类的总类数
			...
			std::cout << "cloud_obstacle.size(): " << cluster_index.size() << endl;
			for(int j=0; j<cluster_index.size(); ++j){//参与聚类的所有点的数量
				if(cluster_index[j] == cluster_id[k]){
					...
					int row = 63 - cloud_obstacle->points[j].ring;
					image.at<cv::Vec3b>(row,col) = cv::Vec3b(b,g,r);
				}
			}
			cloudview.push_back(Colorcloud2);
		}
		std::cout << "cloud_ground.size(): " << cloud_ground->points.size() << endl;
		
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr Colorcloud3(
				new pcl::PointCloud<pcl::PointXYZRGB>);
		for(int i = 0; i < cloud_ground->points.size(); ++i){

				pcl::PointXYZRGB p2;
				p2.x = cloud_ground->points[i].x;
				p2.y = cloud_ground->points[i].y;
				p2.z = cloud_ground->points[i].z;
				p2.r = 255;
				p2.g = 0;
				p2.b = 255;
				Colorcloud3->points.push_back(p2);
		}
		cloudview.push_back(Colorcloud3);
		...
		for(int i = 0; i < cloud_ground->points.size(); ++i){
			...
			int row = 63 - cloud_ground->points[i].ring;
			image.at<cv::Vec3b>(row,col) = cv::Vec3b(b,g,r);
		}
